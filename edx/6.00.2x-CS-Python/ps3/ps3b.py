...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    viruses = [SimpleVirus(maxBirthProb, clearProb) for v in xrange(numViruses)]    

    for trial in xrange(numTrials):
        patient = Patient(viruses, maxPop)
        for i in xrange(steps):
            trialResults[i].append(patient.update())

    res = [sum(l) / float(numTrials) for l in trialResults]
    pylab.title('SimpleVirus Simulation')
    pylab.plot(res)
    pylab.legend('A Curve')
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            if not self.resistances[drug]: return NoChildException
        
        if random.random() < self.maxBirthProb * (1 - popDensity):
            childRes = {}
            for drug in self.resistances:
                if random.random() <= 1 - self.mutProb:
                    childRes[drug] = self.resistances[drug]
                else:
                    childRes[drug] = not self.resistances[drug]
                return ResistantVirus(
                    self.maxBirthProb, self.clearProb, childRes, self.mutProb
                )
        else:
            raise NoChildException
